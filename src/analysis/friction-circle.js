...
export class FrictionCircleAnalyzer {
  ...
  findMaxG() {
    ...
    for (const s of this.samples) {
      const lateral = Math.abs(s.motion?.acceleration?.lateralG || 0);
      const longitudinal = Math.abs(s.motion?.acceleration?.longitudinalG || 0);
  ...
  generateFrictionCircle() {
    ...
    for (const s of this.samples) {
      const latG = s.motion?.acceleration?.lateralG || 0;
      const longG = s.motion?.acceleration?.longitudinalG || 0;
