...
describe('Electron Desktop Integration & Network Helpers', () => {
  ...
  test('Auto-Archive & User Reports Directory: Resolves path to Documents/APEX/user', async () => {
    const path = await import('node:path');
    const os = await import('node:os');
    const docsDir = path.join(os.homedir(), 'Documents');
    const targetUserDir = path.join(docsDir, 'APEX', 'user');
    const targetProfilesDir = path.join(targetUserDir, 'Profiles');

    assert.ok(targetUserDir.includes('APEX'), 'User archive path must contain APEX');
    assert.ok(targetUserDir.endsWith(path.join('APEX', 'user')), 'Must resolve to Documents/APEX/user');
    assert.ok(targetProfilesDir.endsWith(path.join('APEX', 'user', 'Profiles')), 'Must resolve to Documents/APEX/user/Profiles');
  });
