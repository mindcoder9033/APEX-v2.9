...
export const STINTS_DATABASE = [
  ...
  // --- TIER 3: REAL-WORLD LINE & ADAPTATION ---
  {
    ...
    name: 'The Real-World Line: Adaptation',
    subtitle: 'Speed-Sensitive Apex Diagnosis, Camber/Elevation Dynamics & Corner Compromise (Type I-III)',
    focus: 'Synthesis of Early Mistake Recognition, Banking/Grip Exploitation & Straightaway Prioritization',
    targetMetric: 'Composite Real-World Mastery >= 85% [Apex Recog: <30ft, Banking Gain: +10% G, Type I Exit: +4.0 MPH]',
    prescribedCar: '2015 Chevrolet Corvette Z06',
    ...
    laps: 12,
    drivatars: 0,
    quote: '"The faster you go, the earlier you have to identify and cure mistakes... The difference between the world class driver and the average racer is that the champions recognize their mistakes sooner." — Mario Andretti (Page 38)',
    briefing: 'Execute a comprehensive 12-lap real-world adaptation session synthesizing the three core racecraft principles from Chapter 3: 1) Speed of Recognition: Maintain a sharp visual Sight Picture to diagnose early turn-in errors 90 feet before the corner and immediately apply the "Relax Steering + Firm Brake" correction to save the late apex. 2) Beyond Geometry: Read the road surface and dynamically exploit positive camber and compression bowls (The Uphill & Downhill) to extract +10% additional cornering grip while avoiding off-camber slip. 3) Corner Compromise: Grade interconnected turns (The Esses) and consciously sacrifice Type III entry speed to position the car for maximum full-throttle launch onto the main straight (Type I).',
    actionPlan: [
      'Early Apex Diagnosis: Maintain an active visual Sight Picture looking far ahead; if trajectory drifts early, immediately relax steering and brake firmly to re-align vehicle attitude toward a safe late apex.',
      'Camber & Elevation Exploitation: Read road camber dynamically — turn in earlier and load the chassis aggressively into positive-camber compression bowls (+10% G-gain) while exercising patience over off-camber crests.',
      'Corner Grading & Compromise: In interconnected corner complexes (The Esses), sacrifice entry speed into Type III turns to position the car wide, commit to early full throttle, and maximize launch velocity onto the main straight.'
    ],
    hudWidgets: ['Early Apex & Attitude Vector', 'Camber & Banking Dynamic G-Gain', 'Corner Priority & Type I Launch Delta', 'Composite Real-World Mastery Index']
  },
  ...
  // --- TIER 4: MASTERING CAR CONTROL ---
  {
    ...
    subtitle: 'Over-Rotation (CPR), Corner Exit Throttle Squeeze & Turn-In Understeer Breathe',
    focus: 'Synthesis of Skid Recovery (C/P/R), Progressive Throttle Feed & Front Tire Loading',
    ...
    laps: 15,
    drivatars: 0,
    quote: '"Controlling the car from the turn-in point of the corner to the throttle application point involves mixing braking loads with cornering loads... The role of car control is to keep the car on your intended path at a yaw angle in the range that creates the best grip from the tires." — Skip Barber (Page 72)',
    briefing: 'Execute a comprehensive 15-lap car control masterclass synthesizing the three core vehicle control disciplines from Chapter 4: 1) Over-Rotation & Skid Recovery (Turn 7): When trail-braking deep rotates the rear axle, execute the strict three-step Correction, Pause, Recovery (CPR) sequence — catch the slide, hold opposite lock in the "eye of the storm" as yaw rate pauses at 0°/s, and rapidly unwind steering to center to eliminate secondary counter-spins. 2) Exit Throttle Squeeze (The Carousel): Avoid abrupt on/off throttle stomping that spikes rear slip angle to 16°; progressively feed power across 15–18 meters to keep slip in the optimal 7°–10° neutral grip band and gain +3.2 km/h exit speed. 3) Understeer Breathe (The Chicane): Approach fast turns by "breathing" throttle to 60–70% an instant before turn-in to load the front tires, resisting the temptation to add excessive steering lock ("More Steering = Less Grip").',
    actionPlan: [
      'CPR Skid Recovery: When the rear rotates past 8° yaw, apply swift opposite lock (Correct), hold steady through the eye of the storm (Pause), and quickly unwind back to center (Recover) to prevent reverse tankslappers.',
      'Exit Throttle Squeeze: Progressively feed throttle across 15–18 meters onto long straights, restraining rear slip angle under 10° to maximize forward acceleration and gain +3.2 km/h exit speed.',
      'Turn-In Throttle Breathe: In fast directional changes, breathe throttle to 60–70% an instant before turn-in to transfer load forward; never force more steering lock when the nose pushes.'
    ],
    hudWidgets: ['Yaw & Slip Angle Balance (7°-10°)', 'Exit Squeeze Distance (Meters) & km/h Delta', 'Turn-In Throttle Breathe %', 'Composite Car Control Index']
  },
  ...
  // --- TIER 5: BRAKING & ENTERING (THE ANALYTICAL BRAKER) ---
  {
    ...
    name: 'The Analytical Braker: Braking & Entering',
    subtitle: '4-Block Corner Entry, Threshold Modulation (140->100 lbs), Friction Circle Blending & "The Procedure"',
    focus: 'Synthesis of Straight-Line Decel, Brake-Turn Traction Circle Blending & Jeremy Dale\'s 3-Ft Progression',
    targetMetric: 'Composite Braking Mastery >= 85% [Modulation: 30-40 lbs Drop (0 Panic Lifts) | Trail: >75% Grip | Procedure: ±3 ft / +2.5+ km/h]',
    ...
    laps: 15,
    drivatars: 0,
    quote: '"It is amazing how many drivers... think that the brakes are for slowing the car down... To unlock a locked tire, the brake pedal pressure needs to be released from approximately 140 lbs down to 100 lbs. The foot motion required is barely perceptible to the naked eye." — Mario Andretti & Skip Barber (Page 74, 76)',
    briefing: 'Execute a comprehensive 15-lap braking masterclass synthesizing the three core corner-entry disciplines from Chapter 5: 1) Block 1 & 2 (Threshold Modulation): Squeeze the brake pedal hard in <0.35s to establish 125–140 lbs threshold force; if a lockup occurs, relax ankle muscle tension by 30–40 lbs without panic-lifting to 0 lbs. 2) Block 3 (Trail-Braking & Traction Circle): Carry braking past turn-in, bleeding off ~20 lbs per 0.10s to keep tire grip on the outer boundary of the top-right Brake-Turn quadrant. 3) Block 4 ("The Procedure" & TAP Exit Speed): Methodically advance your braking point 3 feet closer each lap until the entry speed delays your Throttle Application Point (TAP) or drops exit speed (+2.5+ km/h).',
    actionPlan: [
      'Threshold Modulation (Block 1 & 2): Squeeze into the braking zone in <0.35s to hit the 125–140 lbs threshold band; modulate lower leg/ankle tension to release 30–40 lbs upon lockup rather than dumping pedal pressure to 0 lbs.',
      'Trail-Braking & Traction Circle (Block 3): Uniformly bleed off brake pressure in direct proportion to steering lock (~20 lbs per 0.10s decay), riding the outer boundary of Mark Donohue\'s Friction Circle.',
      'The Procedure & Exit Speed (Block 4): Step brake markers 3 feet closer each lap; verify that braking deeper does not compromise corner exit speed (+2.5+ km/h) or delay throttle pick-up through "The Pause".'
    ],
